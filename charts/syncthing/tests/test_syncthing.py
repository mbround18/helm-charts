...
def test_argocd_metadata_is_not_rendered_by_default():
    ...
    http_service = next(document for document in documents if document.get("metadata", {}).get("name", "").endswith("-http"))
...
def test_argocd_metadata_renders_when_application_api_is_available():
    ...
    services = [
        document for document in documents if isinstance(document, dict) and document.get("kind") == "Service"
    ]
    ...
    assert {service["metadata"]["annotations"]["argocd.argoproj.io/sync-wave"] for service in services} == {"40"}
...
def test_argocd_force_mode_adds_instance_label(password_secret):
    ...
    assert statefulset["metadata"]["labels"]["argocd.argoproj.io/instance"] == "syncthing-prod"
